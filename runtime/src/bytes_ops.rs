//! Bytes operations FFI functions for the Coral runtime.
...
pub extern "C" fn coral_bytes_get(value: ValueHandle, index: ValueHandle) -> ValueHandle {
    if value.is_null() || index.is_null() { return coral_make_number(0.0); }
    let v = unsafe { &*value };
    let iv = unsafe { &*index };
    if v.tag != ValueTag::Bytes as u8 { return coral_make_number(0.0); }
    if iv.tag != ValueTag::Number as u8 { return coral_make_number(0.0); }
...
pub extern "C" fn coral_bytes_from_string(s: ValueHandle) -> ValueHandle {
    if s.is_null() { return coral_make_bytes(ptr::null(), 0); }
    let v = unsafe { &*s };
    if v.tag != ValueTag::String as u8 { return coral_make_bytes(ptr::null(), 0); }
    let bytes = string_to_bytes(v);
...
pub extern "C" fn coral_bytes_to_string(b: ValueHandle) -> ValueHandle {
    if b.is_null() { return coral_make_string(ptr::null(), 0); }
    let v = unsafe { &*b };
    if v.tag != ValueTag::Bytes as u8 { return coral_make_string(ptr::null(), 0); }
    let bytes = string_to_bytes(v);
    // Attempt UTF-8 conversion; use replacement characters if invalid
...
#[unsafe(no_mangle)]
pub extern "C" fn coral_bytes_slice_val(value: ValueHandle, start: ValueHandle, end: ValueHandle) -> ValueHandle {
    if value.is_null() { return coral_make_bytes(ptr::null(), 0); }
    let v = unsafe { &*value };
    if v.tag != ValueTag::Bytes as u8 { return coral_make_bytes(ptr::null(), 0); }
    let bytes = string_to_bytes(v);
    let len = bytes.len();
...
/// Parse a hex string into a Bytes value.
#[unsafe(no_mangle)]
pub extern "C" fn coral_bytes_from_hex(value: ValueHandle) -> ValueHandle {
    ...
    if bytes.len() % 2 != 0 { return coral_make_absent(); }
    let mut i = 0;
    while i < bytes.len() {
        let hi = match hex_val(bytes[i]) { Some(v) => v, None => return coral_make_absent() };
        let lo = match hex_val(bytes[i+1]) { Some(v) => v, None => return coral_make_absent() };
        result.push((hi << 4) | lo);
        i += 2;
    }
...
/// Check if a bytes value contains a sub-sequence.
#[unsafe(no_mangle)]
pub extern "C" fn coral_bytes_contains(haystack: ValueHandle, needle: ValueHandle) -> ValueHandle {
    if haystack.is_null() || needle.is_null() { return coral_make_bool(0); }
    let hv = unsafe { &*haystack };
    let nv = unsafe { &*needle };
    let h = string_to_bytes(hv);
    let n = string_to_bytes(nv);
    if n.is_empty() { return coral_make_bool(1); }
    for window in h.windows(n.len()) {
        if window == n.as_slice() { return coral_make_bool(1); }
    }
    coral_make_bool(0)
}

/// Find the offset of a sub-sequence in a bytes value, or -1 if not found.
#[unsafe(no_mangle)]
pub extern "C" fn coral_bytes_find(haystack: ValueHandle, needle: ValueHandle) -> ValueHandle {
    if haystack.is_null() || needle.is_null() { return coral_make_number(-1.0); }
    let hv = unsafe { &*haystack };
    let nv = unsafe { &*needle };
    let h = string_to_bytes(hv);
    let n = string_to_bytes(nv);
    if n.is_empty() { return coral_make_number(0.0); }
    for (i, window) in h.windows(n.len()).enumerate() {
        if window == n.as_slice() { return coral_make_number(i as f64); }
    }
