...
use super::{SharedStoreEngine, close_engine, open_store_engine, save_all_engines};
    ...
    unsafe {
        match value {
            ...
            StoredValue::List(items) => {
                let handles: Vec<ValueHandle> =
                    items.iter().map(|v| stored_value_to_handle(v)).collect();
    ...
    match tag {
        ...
        TAG_MAP => {
            ...
            for i in 0..keys_len {
                ...
                let stored_val = handle_to_stored_value(val_handle).unwrap_or(StoredValue::Unit);
                pairs.push((key_str, stored_val));
            }
            unsafe {
                coral_value_release(keys_list);
            }
            Some(StoredValue::Map(pairs))
        }
        _ => Some(StoredValue::Unit),
    }
}
...
pub extern "C" fn coral_store_get_by_index(handle: ValueHandle, index: ValueHandle) -> ValueHandle {
    ...
    match with_engine(handle, |engine| engine.get(idx).map_err(|e| e.to_string())) {
        Ok(Some(obj)) => create_object_map(
            obj.index,
            &obj.uuid.to_string(),
            obj.version,
            obj.created_at,
...
pub extern "C" fn coral_store_get_by_uuid(
    ...
    match with_engine(handle, |engine| {
        engine.get_by_uuid(&uuid).map_err(|e| e.to_string())
    }) {
        Ok(Some(obj)) => create_object_map(
            obj.index,
            &obj.uuid.to_string(),
...
pub extern "C" fn coral_store_soft_delete(handle: ValueHandle, index: ValueHandle) -> ValueHandle {
    ...
    match with_engine(handle, |engine| {
        engine.delete(idx).map_err(|e| e.to_string())
    }) {
...
pub extern "C" fn coral_store_stats(handle: ValueHandle) -> ValueHandle {
    match with_engine(handle, |engine| Ok(engine.stats())) {
        Ok(stats) => unsafe {
...
pub extern "C" fn coral_store_count(handle: ValueHandle) -> ValueHandle {
    match with_engine(handle, |engine| Ok(engine.count())) {
...
pub extern "C" fn coral_store_persist(handle: ValueHandle) -> ValueHandle {
    match with_engine(handle, |engine| engine.save().map_err(|e| e.to_string())) {
        Ok(()) => unsafe { coral_make_bool(1) },
        Err(e) => e,
    }
}
...
pub extern "C" fn coral_store_all_indices(handle: ValueHandle) -> ValueHandle {
    match with_engine(handle, |engine| Ok(engine.all())) {
        Ok(indices) => unsafe {
            let handles: Vec<ValueHandle> = indices
                .iter()
...
pub extern "C" fn coral_store_compact(handle: ValueHandle) -> ValueHandle {
    match with_engine(handle, |engine| {
        engine.compact_wal().map_err(|e| e.to_string())
    }) {
...
pub extern "C" fn coral_store_create_index(
    ...
    match with_engine(handle, |engine| {
        engine
            .create_secondary_index(&field, idx_kind)
            .map_err(|e| e.to_string())
    }) {
        Ok(()) => unsafe { coral_make_bool(1) },
        Err(e) => e,
...
pub extern "C" fn coral_store_drop_index(
    ...
    let field = extract_string(field_name);
    match with_engine(handle, |engine| Ok(engine.drop_secondary_index(&field))) {
...
pub extern "C" fn coral_store_find_by_field(
    ...
    match with_engine(handle, |engine| {
        engine
            .find_by_field(&field, &stored_val)
            .map_err(|e| e.to_string())
    }) {
        Ok(ids) => unsafe {
            let handles: Vec<ValueHandle> =
                ids.iter().map(|id| coral_make_number(*id as f64)).collect();
            let result = coral_make_list(handles.as_ptr(), handles.len());
            for h in handles {
                coral_value_release(h);
            }
...
pub extern "C" fn coral_store_find_by_range(
    ...
    match with_engine(handle, |engine| {
        engine
            .find_by_field_range(&field, &min_val, &max_val)
            .map_err(|e| e.to_string())
    }) {
        Ok(ids) => unsafe {
            let handles: Vec<ValueHandle> =
...
pub extern "C" fn coral_store_indexed_fields(handle: ValueHandle) -> ValueHandle {
    match with_engine(handle, |engine| Ok(engine.indexed_fields())) {
        Ok(fields) => unsafe {
            let handles: Vec<ValueHandle> = fields
                .iter()
...
mod tests {
    ...
    fn test_stored_value_conversions() {
        ...
        let list_val = StoredValue::List(vec![StoredValue::Int(1), StoredValue::Int(2)]);
    ...
    fn test_handle_registry() {
        let id1 = register_handle(
            "TestType".to_string(),
            "default".to_string(),
            ".coral_data".to_string(),
        );
        let id2 = register_handle(
            "TestType".to_string(),
            "other".to_string(),
            "/tmp/test".to_string(),
        );
