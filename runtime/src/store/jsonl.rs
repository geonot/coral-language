//! JSON Lines storage format for Coral persistent stores
//!
//! Format (data.jsonl):
//! - One JSON object per line
//! - Each line is a complete, self-contained record
//! - System attributes are prefixed with underscore
//!
//! Example:
//! {"_index":1,"_uuid":"01905e9a-...","_created_at":1719849600000,"name":"Alice"}
...
impl JsonlWriter {
    ...
    pub fn open(path: impl AsRef<Path>) -> io::Result<Self> {
        ...
        let (file, write_offset, line_count) = if path.exists() {
            ...
            // Count existing lines and find write offset
...
/// A record read from JSONL storage
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn test_json_parser() {
        ...
        assert!(matches!(record.fields.get("numbers"), Some(StoredValue::List(_))));
        assert!(matches!(record.fields.get("nested"), Some(StoredValue::Map(_))));
    }
