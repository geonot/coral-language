...
impl Uuid7 {
    ...
    pub fn from_timestamp(timestamp_ms: u64) -> Self {
        ...
        bytes[8] = 0x80 | ((random >> 56) & 0x3F) as u8;
    ...
    pub fn parse(s: &str) -> io::Result<Self> {
        ...
        for i in 0..16 {
            bytes[i] = u8::from_str_radix(&hex[i * 2..i * 2 + 2], 16).map_err(|_| {
                io::Error::new(io::ErrorKind::InvalidData, "invalid hex character in UUID")
            })?;
        }
...
mod tests {
    ...
    fn test_uuid7_from_bytes() {
        let bytes = [
            0x01, 0x8e, 0x28, 0xa5, 0x7c, 0x10, 0x70, 0x00, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00,
        ];
