//! Cryptographic operations for the Coral runtime (L3.4).
//!
//! Provides SHA-256, HMAC-SHA256, AES-256-CBC, and OS-entropy random bytes.
//! All crypto is implemented using pure Rust (no OpenSSL dependency).
...
fn sha256_digest(data: &[u8]) -> [u8; 32] {
    ...
    let mut last_blocks = vec![0u8; 128]; // at most 2 blocks
...
fn aes256_key_expansion(key: &[u8; 32]) -> [[u8; 16]; 15] {
    ...
    let nk = 8; // AES-256 key words
    let nr = 14; // AES-256 rounds
...
fn aes256_encrypt_block(block: &[u8; 16], round_keys: &[[u8; 16]; 15]) -> [u8; 16] {
    ...
    for round in 1..14 {
        ...
        // MixColumns
        for c in 0..4 {
            let s0 = state[c*4]; let s1 = state[c*4+1]; let s2 = state[c*4+2]; let s3 = state[c*4+3];
...
fn os_random_bytes(n: usize) -> Vec<u8> {
    ...
    #[cfg(not(unix))]
    {
        // Fallback: use thread_rng-style approach
...
fn safe_string(h: ValueHandle) -> Option<String> {
    ...
    let val = unsafe { &*h };
    if val.tag != ValueTag::String as u8 { return None; }
    Some(value_to_rust_string(val))
}

/// `coral_sha256(data_string) -> hex_string`
#[unsafe(no_mangle)]
...
/// `coral_hmac_sha256(key_hex, message_string) -> hex_string`
#[unsafe(no_mangle)]
pub extern "C" fn coral_hmac_sha256(key_handle: ValueHandle, msg_handle: ValueHandle) -> ValueHandle {
    ...
    let key_bytes = match hex_to_bytes(&key_str) {
        Some(b) => b,
        None => key_str.as_bytes().to_vec(), // treat as raw key if not valid hex
    };
    let mac = hmac_sha256(&key_bytes, msg.as_bytes());
    coral_make_string_from_rust(&bytes_to_hex(&mac))
}

/// `coral_aes256_encrypt(plaintext, key_hex, iv_hex) -> ciphertext_hex`
#[unsafe(no_mangle)]
...
/// `coral_aes256_decrypt(ciphertext_hex, key_hex, iv_hex) -> plaintext | ""`
#[unsafe(no_mangle)]
...
/// `coral_random_bytes(n) -> hex_string`
#[unsafe(no_mangle)]
pub extern "C" fn coral_random_bytes(n_handle: ValueHandle) -> ValueHandle {
    let n = if n_handle.is_null() {
        ...
        if val.tag != ValueTag::Number as u8 { 16 } else { (unsafe { val.payload.number }) as usize }
    };
    let n = n.min(1024); // cap at 1KB
