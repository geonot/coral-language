...
- **Backend:** LLVM IR emission for literals, lists/maps, template strings, taxonomy literals, logical/match lowering; main runs as an actor supervisor and dispatches to `__user_main`.
...
- Store/actor state layout and field access; typed message contracts; backpressure, supervision policies, timers.
...
- Top-level: bindings, `*fn` functions, `type`, `store`, `actor` (parsed), `use module.path`.
