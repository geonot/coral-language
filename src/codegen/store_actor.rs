...
impl<'ctx> CodeGenerator<'ctx> {
    pub(super) fn build_actor_constructor(
        &mut self,
        ...
        let state_map = self.call_runtime_ptr(
        ...
        let handler_invoke_fn =
            self.module
                .add_function(&handler_fn_name, self.runtime.closure_invoke_type, None);
        self.build_actor_handler_invoke(handler_invoke_fn, store)?;

        let release_fn_name = format!("__{}_handler_release", store.name);
        let release_fn =
            self.module
                .add_function(&release_fn_name, self.runtime.closure_release_type, None);
        ...
        let actor_handle =
            self.call_runtime_ptr(self.runtime.actor_spawn, &[handler_closure.into()], "actor");
    ...
    pub(super) fn build_actor_handler_invoke(
        ...
        let state = self
            .builder
            .build_pointer_cast(env_param, self.runtime.value_ptr_type, "actor_state")
            .unwrap();
        ...
        let msg_param = self
            .builder
            .build_load(self.runtime.value_ptr_type, msg_ptr, "msg")
            .unwrap()
            .into_pointer_value();
        ...
        for method in &handlers {
            ...
            let str_ptr = self
                .builder
                .build_pointer_cast(str_global.as_pointer_value(), i8_ptr_type, "str_ptr")
                .unwrap();
        ...
        for (i, (ptr_val, len_val)) in ptr_values.iter().zip(len_values.iter()).enumerate() {
            ...
            let ptr_slot = unsafe {
                self.builder
                    .build_in_bounds_gep(i8_ptr_type, ptrs_array, &[idx], "ptr_slot")
                    .unwrap()
            };
            self.builder.build_store(ptr_slot, *ptr_val).unwrap();
            let len_slot = unsafe {
                self.builder
                    .build_in_bounds_gep(self.usize_type, lens_array, &[idx], "len_slot")
                    .unwrap()
            };
        ...
        for (i, method) in handlers.iter().enumerate() {
            let case_bb = self
                .context
                .append_basic_block(invoke_fn, &format!("msg_{}", method.name));
        ...
        self.builder.position_at_end(handler_entry);
        self.builder
            .build_switch(dispatch_result, default_bb, &cases)
            .unwrap();
    ...
    pub(super) fn build_actor_handler_release(&mut self, release_fn: FunctionValue<'ctx>) {
        ...
        let state = self
            ...
            .unwrap();
        self.builder
            .build_call(self.runtime.value_release, &[state.into()], "")
            .unwrap();
    ...
    pub(super) fn build_store_constructor(
        &mut self,
        store: &crate::ast::StoreDefinition,
    ) -> Result<(), Diagnostic> {
        let constructor_name = format!("make_{}", store.name);
        ...
        if store.is_persistent {
            ...
            let type_ptr = self
                .builder
                .build_pointer_cast(type_global.as_pointer_value(), i8_ptr_type, "type_ptr")
                .unwrap();
            ...
            let name_ptr = self
                .builder
                .build_pointer_cast(name_global.as_pointer_value(), i8_ptr_type, "name_ptr")
                .unwrap();
            let name_len = self.usize_type.const_int(7, false);

            let path_global = self.get_or_create_string_constant(".coral_data");
            let path_ptr = self
                .builder
                .build_pointer_cast(path_global.as_pointer_value(), i8_ptr_type, "path_ptr")
                .unwrap();
            let path_len = self.usize_type.const_int(11, false);
