...
fn e2e_stderr_write() {
    ...
    let (stdout, stderr, code) = run_coral(
        r#"
*main()
...
"#,
    ...
    assert!(
        ...
        "Expected stderr to contain 'hello stderr' but got: {}",
        stderr
    );
    assert!(
        stdout.contains("done"),
        "Expected stdout to contain 'done' but got: {}",
        stdout
    );
}
...
fn e2e_cc53_hello_example() {
    let source = std::fs::read_to_string(PathBuf::from(WORKSPACE).join("examples/hello.coral"))
...
fn e2e_cc53_fizzbuzz_example() {
    let source = std::fs::read_to_string(PathBuf::from(WORKSPACE).join("examples/fizzbuzz.coral"))
        .expect("read fizzbuzz.coral");
    let (stdout, _stderr, code) = run_coral(&source);
    assert_eq!(code, 0, "fizzbuzz.coral should exit 0");
    assert!(
        stdout.contains("FizzBuzz"),
        "fizzbuzz.coral should contain FizzBuzz"
    );
    assert!(
        stdout.contains("Fizz"),
        "fizzbuzz.coral should contain Fizz"
    );
    assert!(
        stdout.contains("Buzz"),
        "fizzbuzz.coral should contain Buzz"
    );
}
...
fn e2e_cc53_calculator_example() {
    let source =
        std::fs::read_to_string(PathBuf::from(WORKSPACE).join("examples/calculator.coral"))
            .expect("read calculator.coral");
    let (stdout, _stderr, code) = run_coral(&source);
    assert_eq!(code, 0, "calculator.coral should exit 0");
    assert!(
        stdout.contains("Calculator demo complete!"),
        "calculator.coral should complete"
    );
}
...
fn e2e_cc53_data_pipeline_example() {
    let source =
        std::fs::read_to_string(PathBuf::from(WORKSPACE).join("examples/data_pipeline.coral"))
            .expect("read data_pipeline.coral");
    let (stdout, _stderr, code) = run_coral(&source);
    assert_eq!(code, 0, "data_pipeline.coral should exit 0");
    assert!(
        stdout.contains("Demo complete!"),
        "data_pipeline.coral should complete"
    );
}
...
fn e2e_cc53_traits_demo_example() {
    let source =
        std::fs::read_to_string(PathBuf::from(WORKSPACE).join("examples/traits_demo.coral"))
            .expect("read traits_demo.coral");
    let (stdout, _stderr, code) = run_coral(&source);
    assert_eq!(code, 0, "traits_demo.coral should exit 0");
    assert!(
        stdout.contains("Demo complete!"),
        "traits_demo.coral should complete"
    );
}
...
"#;
    ...
    assert!(
        ws.iter()
            .any(|w| w.contains("may return 'none'") && w.contains("lookup")),
        "expected nullability warning for 'lookup', got: {:?}",
        ws
    );
}
...
"#;
    ...
    assert!(
        ...
        "did NOT expect nullability warning, got: {:?}",
        ws
    );
}
...
"#;
    ...
    assert!(
        ws.iter()
            .any(|w| w.contains("may return 'none'") && w.contains("find")),
        "expected nullability warning for 'find', got: {:?}",
        ws
    );
}
...
"#;
    ...
    assert!(
        !ws.iter()
            .any(|w| w.contains("may return 'none'") && w.contains("main")),
        "should not warn on main(), got: {:?}",
        ws
    );
}
...
"#,
    ...
    assert!(
        result.is_err(),
        "do without end should produce a parse error"
    );
}
...
"#,
    ...
    assert!(
        ...
        "optimized IR should contain function definitions"
    );
}
...
"#,
    ...
    assert_eq!(
        stdout.trim(),
        "30",
        "optimized code should produce correct output"
    );
}
