...
use coralc::types::core::{Primitive, TypeId};
...
fn type_env_nested_type_parameters() {
    ...
    // Verify T is bound
    assert_eq!(
        env.get_type_param("T"),
        Some(&TypeId::Primitive(Primitive::String))
    );
...
fn module_loader_detects_circular_imports() {
    ...
    assert!(
        err.contains("Hint:") || err.contains("restructur"),
        "should have hint: {}",
        err
    );
    assert!(
...
"#,
    ...
    assert!(
        info.exports.contains(&"Point".to_string()),
        "should export Point type"
    );
    assert!(
        info.exports.contains(&"Color".to_string()),
        "should export Color type"
    );
    assert!(
        info.exports.contains(&"Counter".to_string()),
        "should export Counter store"
    );
    assert!(
        info.exports.contains(&"Logger".to_string()),
        "should export Logger actor"
    );
    assert!(
        info.exports.contains(&"Serializable".to_string()),
        "should export Serializable trait"
    );
    assert!(
        info.exports.contains(&"helper".to_string()),
        "should export helper function"
    );
}
...
fn module_loader_handles_diamond_imports() {
    ...
    assert_eq!(
        shared_count, 1,
        "shared function should appear exactly once"
    );
}
